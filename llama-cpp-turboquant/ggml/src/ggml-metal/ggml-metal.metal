...
// --- Midpoints for nearest-centroid lookup during quantization ---
...
static void turbo_rotate_forward(thread float * x, constant float * s1, constant float * s2) {
    const float inv_sqrt = 0.088388347f; // 1/sqrt(128)
    for (int i = 0; i < 128; i++) x[i] *= s1[i];
    turbo_fwht_128(x);
    for (int i = 0; i < 128; i++) x[i] *= s2[i] * inv_sqrt;
}
...
static void turbo_rotate_inverse(thread float * x, constant float * s1, constant float * s2) {
    ...
    turbo_fwht_128(x);
    for (int i = 0; i < 128; i++) x[i] *= s1[i] * inv_sqrt;
}
...
#pragma METAL fp math_mode(safe)
    // 4-bit PolarQuant (Natural Space): normalize → quantize to 16 centroids → nibble pack
    float norm_sq = 0.0f;
...
void dequantize_turbo2_0(device const block_turbo2_0 * xb, short il, thread type4x4 & reg) {
    const float norm_b = float(xb->norm);
    const int off = il * 16;
    float4x4 res;
    for (int g = 0; g < 4; g++) {
        const int goff = off + g * 4;
        const uint8_t qb = xb->qs[goff / 4];
        res[g] = float4(
            float(turbo_centroids_2bit_h[(qb      ) & 0x03]),
            float(turbo_centroids_2bit_h[(qb >> 2) & 0x03]),
            float(turbo_centroids_2bit_h[(qb >> 4) & 0x03]),
            float(turbo_centroids_2bit_h[(qb >> 6)       ])
        ) * norm_b;
    }
    reg = (type4x4) res;
}
...
void dequantize_turbo2_0_t4(device const block_turbo2_0 * xb, short il, thread type4 & reg) {
    ...
    // Simplest possible centroid-only dequant (Natural Space)
    reg = type4(
        float(turbo_centroids_2bit_h[(xb->qs[off/4] >> ((off%4)*2)) & 0x03]),
        float(turbo_centroids_2bit_h[(xb->qs[(off+1)/4] >> (((off+1)%4)*2)) & 0x03]),
        float(turbo_centroids_2bit_h[(xb->qs[(off+2)/4] >> (((off+2)%4)*2)) & 0x03]),
        float(turbo_centroids_2bit_h[(xb->qs[(off+3)/4] >> (((off+3)%4)*2)) & 0x03])
    ) * norm_b;
}

template <typename type4>
void dequantize_turbo2_0_t4_k(device const block_turbo2_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo2_0_t4(xb, il, reg);
}

// Natural-space K dequant: K is stored WHT-rotated, Q is also WHT-rotated in graph.
// _k_nat returns rotated values — same as _t4_k — ensuring Q_rot^T @ K_rot is correct.
template <typename type4>
void dequantize_turbo2_0_t4_k_nat(device const block_turbo2_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo2_0_t4_k(xb, il, reg);
}
...
void dequantize_turbo3_0(device const block_turbo3_0 * xb, short il, thread type4x4 & reg) {
    ...
    for (int g = 0; g < 4; g++) {
        const int j = off + g * 4;
        const uint8_t q0 = (xb->qs[j/4] >> ((j%4)*2)) & 0x3;
        const uint8_t s0 = (xb->signs[j/8] >> (j%8)) & 1;
        const uint8_t q1 = (xb->qs[(j+1)/4] >> (((j+1)%4)*2)) & 0x3;
        const uint8_t s1 = (xb->signs[(j+1)/8] >> ((j+1)%8)) & 1;
        const uint8_t q2 = (xb->qs[(j+2)/4] >> (((j+2)%4)*2)) & 0x3;
        const uint8_t s2 = (xb->signs[(j+2)/8] >> ((j+2)%8)) & 1;
        const uint8_t q3 = (xb->qs[(j+3)/4] >> (((j+3)%4)*2)) & 0x3;
        const uint8_t s3 = (xb->signs[(j+3)/8] >> ((j+3)%8)) & 1;
        res[g] = float4(
...
void dequantize_turbo3_0_t4(device const block_turbo3_0 * xb, short il, thread type4 & reg) {
    ...
    for (int i = 0; i < 4; i++) {
        const int j = off + i;
        const uint8_t q0 = (xb->qs[j/4] >> ((j%4)*2)) & 0x3;
        const uint8_t s0 = (xb->signs[j/8] >> (j%8)) & 0x1;
        res[i] = float(turbo_centroids_3bit_h[q0 | (s0 << 2)]);
    }
    reg = type4(res * norm_b);
}

// K-cache variant: same as _t4 (K is stored in WHT-rotated space, Q is rotated in graph)
template <typename type4>
void dequantize_turbo3_0_t4_k(device const block_turbo3_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo3_0_t4(xb, il, reg);
}

// Natural-space K dequant: K is stored WHT-rotated, Q is also WHT-rotated in graph.
// So _k_nat returns rotated values — same as _t4_k — ensuring Q_rot^T @ K_rot is correct.
template <typename type4>
void dequantize_turbo3_0_t4_k_nat(device const block_turbo3_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo3_0_t4_k(xb, il, reg);
}
...
void dequantize_turbo4_0(device const block_turbo4_0 * xb, short il, thread type4x4 & reg) {
    ...
    for (int g = 0; g < 4; g++) {
        const int goff = off + g * 4;
        const uint8_t qb0 = xb->qs[goff / 2];
        const uint8_t qb1 = xb->qs[(goff + 2) / 2];
        res[g] = float4(
            float(turbo_centroids_4bit_h[(qb0     ) & 0xF]),
            float(turbo_centroids_4bit_h[(qb0 >> 4) & 0xF]),
            float(turbo_centroids_4bit_h[(qb1     ) & 0xF]),
            float(turbo_centroids_4bit_h[(qb1 >> 4) & 0xF])
        ) * norm;
    }
    reg = (type4x4) res;
}
...
void dequantize_turbo4_0_t4(device const block_turbo4_0 * xb, short il, thread type4 & reg) {
    const float norm = float(xb->norm);
    const int off = il * 4;
    const uint8_t qb0 = xb->qs[off / 2];
    const uint8_t qb1 = xb->qs[(off + 2) / 2];
    reg = type4(
        float(turbo_centroids_4bit_h[(qb0     ) & 0xF]),
        float(turbo_centroids_4bit_h[(qb0 >> 4) & 0xF]),
        float(turbo_centroids_4bit_h[(qb1     ) & 0xF]),
...
// FA dequantizers: use standard (raw) dequantization.
// Rotation is handled at the graph level (Q and FA-Output) and storage level (K/V cache).
template <typename type4x4>
void dequantize_turbo4_0_k(device const block_turbo4_0 * xb, short il, thread type4x4 & reg) {
    dequantize_turbo4_0(xb, il, reg);
}

template <typename type4>
void dequantize_turbo4_0_t4_k(device const block_turbo4_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo4_0_t4(xb, il, reg);
}

// turbo4 is quantized in Natural Space (no WHT in set_rows), so _k_nat == _t4_k.
template <typename type4>
void dequantize_turbo4_0_t4_k_nat(device const block_turbo4_0 * xb, short il, thread type4 & reg) {
    dequantize_turbo4_0_t4_k(xb, il, reg);
}
...
kernel void kernel_turbo4_dequant_f16(
    ...
    // CRITICAL: Apply inverse rotation (WHT + sign flip) — DISABLED for Natural Space stability
...
// Matches ggml_compute_forward_turbo_wht_f32 (CPU): group_size 64 or 128, optional InnerQ scale,
// and correct striding when head_dim has multiple groups per row (e.g. two 64-groups in 128-dim).
...
kernel void kernel_turbo_wht(
        ...
        uint tiitg [[thread_index_in_threadgroup]],
        uint ntg   [[threads_per_threadgroup]]) {
    const int32_t gs = args.group_size;
    const int32_t hd = args.head_dim;
    if (gs != 64 && gs != 128) { return; }
    if (hd < gs || (hd % gs) != 0) { return; }

    const int32_t gph = hd / gs;
    const int64_t n_groups = args.n_elements / (int64_t) gs;
    const int64_t group_idx = (int64_t) tgpig * (int64_t) ntg + (int64_t) tiitg;
    if (group_idx >= n_groups) { return; }

    const int64_t head_row   = group_idx / (int64_t) gph;
    const int64_t grp_in_row = group_idx % (int64_t) gph;
    const int64_t base       = head_row * (int64_t) hd + grp_in_row * (int64_t) gs;

    thread float x[128];
    for (int i = 0; i < gs; i++) {
        x[i] = src[base + (int64_t) i];
    }

    const bool is_inverse = (args.direction == 1);
    const bool use_scale  = (args.has_scale != 0);

    if (use_scale && !is_inverse) {
        for (int i = 0; i < gs; i++) {
            x[i] *= scale_inv[i % gs];
        }
    }

    constant float * s_first  = is_inverse ? turbo_wht_signs2 : turbo_wht_signs1;
    constant float * s_second = is_inverse ? turbo_wht_signs1 : turbo_wht_signs2;
    ...
    const float inv_sqrt = 1.0f / sqrt((float) gs);
    for (int i = 0; i < gs; i++) {
        float val = x[i] * inv_sqrt * s_second[i];
        if (use_scale && is_inverse) {
            val *= scale_inv[i % gs];
        }
        dst[base + (int64_t) i] = val;
    }
}
            ...
            // Q*K^T
            {
                ...
                FOR_UNROLL (short cc = 0; cc < C/NE; ++cc) {
                    if (is_same<kd4_t, k4_t>::value) {
                        ...
                        if (is_same<kd4_t, block_turbo3_0>::value) {
                            // FUSED BLOCK DOT: flip computation order for block_turbo3_0 only.
                            ...
                            // Fallback to standard dequantizer for non-turbo3 types
                            k4_t mk;
                            FOR_UNROLL (short ii = 0; ii < DK4/NL; ++ii) {
                                const short i = ii*NL + tx;
...
// Symmetric TurboQuant VEC Flash Attention - Natural Space (k_nat for K-cache)
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk32_dv32")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 32,  32, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk64_dv64")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 64,  64, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk96_dv96")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 96,  96, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo2_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, 32, dequantize_turbo2_0_t4_k_nat, block_turbo2_0, 32, dequantize_turbo2_0_t4, 576, 512, 2>;

template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk32_dv32")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 32, 32, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk64_dv64")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 64, 64, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk96_dv96")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 96, 96, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo3_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 576, 512, 2>;

template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk32_dv32")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 32, 32, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk64_dv64")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 64, 64, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk96_dv96")]]   kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 96, 96, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo4_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo2 K, turbo3 V
// Asymmetric K/V TurboQuant vec flash attention — turbo2 K, turbo3 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo3_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo3 K, turbo2 V
// Asymmetric K/V TurboQuant vec flash attention — turbo3 K, turbo2 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo2_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo2 K, turbo4 V
// Asymmetric K/V TurboQuant vec flash attention — turbo2 K, turbo4 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo2_vturbo4_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo4 K, turbo2 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo2_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo2_0, NL_TURBO2_VEC, dequantize_turbo2_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo3 K, turbo4 V
// Asymmetric K/V TurboQuant vec flash attention — turbo3 K, turbo4 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo3_vturbo4_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4_k_nat, block_turbo4_0, 32, dequantize_turbo4_0_t4, 576, 512, 2>;

// Asymmetric K/V TurboQuant vec flash attention — turbo4 K, turbo3 V
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vturbo3_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_turbo3_0, NL_TURBO3_VEC, dequantize_turbo3_0_t4, 576, 512, 2>;
...
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk32_dv32")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_q8_0, 8, dequantize_q8_0_t4, 32, 32, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk64_dv64")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k_nat, block_q8_0, 8, dequantize_q8_0_t4, 64, 64, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk96_dv96")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 96, 96, 4>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 128, 128, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk192_dv192")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 192, 192, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk192_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 192, 128, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk256_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 256, 256, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk320_dv256")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 320, 256, 2>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk512_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 512, 512, 1>;
template [[host_name("kernel_flash_attn_ext_vec_kturbo4_vq8_0_dk576_dv512")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_turbo4_0, 32, dequantize_turbo4_0_t4_k, block_q8_0, 8, dequantize_q8_0_t4, 576, 512, 2>;
