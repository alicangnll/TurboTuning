...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--turbo-async"},
        {"--no-turbo-async"},
        string_format("enable TurboQuant async 3-stage pipeline (default: %s)", params.turbo_async ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.turbo_async = value;
        }
    ).set_env("LLAMA_ARG_TURBO_ASYNC"));
    add_opt(common_arg(
        {"--sparse-v-threshold"}, "N",
        string_format("attention weight threshold for Sparse-V dequantization (default: %f)", params.sparse_v_thresh),
        [](common_params & params, const std::string & value) {
            params.sparse_v_thresh = std::stof(value);
        }
    ).set_env("LLAMA_ARG_SPARSE_V_THRESHOLD"));
    add_opt(common_arg(
        {"-p", "--prompt"}, "PROMPT",
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (params.cache_type_k == GGML_TYPE_TURBO2_0 || params.cache_type_k == GGML_TYPE_TURBO3_0 || params.cache_type_k == GGML_TYPE_TURBO4_0) {
                params.turbo_async = true;
            }
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (params.cache_type_v == GGML_TYPE_TURBO2_0 || params.cache_type_v == GGML_TYPE_TURBO3_0 || params.cache_type_v == GGML_TYPE_TURBO4_0) {
                params.turbo_async = true;
            }
        }
