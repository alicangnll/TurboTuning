...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    // TurboQuant: Natural Space stability — no inverse WHT needed on attention output
    ggml_build_forward_expand(gf, cur);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // TurboQuant: Use Walsh-Hadamard Rotated Space for high-precision KV compression.
    // Q, K, and V are all rotated into the Gaussian domain.
    if (k->type == GGML_TYPE_TURBO3_0 || k->type == GGML_TYPE_TURBO4_0 || k->type == GGML_TYPE_TURBO2_0) {
        // 1. Pad Q per-head if head_dim < 128
        if (q->ne[0] % 128 != 0) {
            const int64_t pad = 128 - (q->ne[0] % 128);
            q = ggml_cont(ctx0, ggml_pad(ctx0, q, pad, 0, 0, 0));
        } else if (!ggml_is_contiguous(q)) {
            q = ggml_cont(ctx0, q);
        }

        // 2. Forward WHT rotation on Q
        q = ggml_turbo_wht(ctx0, q, 0, 128, nullptr);
    }

    ggml_tensor * cur = build_attn_mha(q, k, v, kq_b, kq_mask, sinks, v_mla, kq_scale, il);

    // 3. Inverse WHT rotation on Attention Output
    if (k->type == GGML_TYPE_TURBO3_0 || k->type == GGML_TYPE_TURBO4_0 || k->type == GGML_TYPE_TURBO2_0) {
