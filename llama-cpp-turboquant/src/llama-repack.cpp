...
bool llama_model_repack_load(struct llama_model * model, const char * filename) {
    ...
    for (int i = 0; i < (int)model->layers.size(); ++i) {
        ...
        auto apply_repack = [&](const char * suffix, ggml_tensor * t) {
            if (!t) return;
            // Skip remapping if tensor is already in a non-host (Metal/GPU) buffer.
            // Metal registers tensors by their original data pointer via newBufferWithBytesNoCopy;
            // changing t->data after registration breaks ggml_metal_buffer_get_id lookups
            // and causes "buffer is nil" errors for every layer on every forward pass.
