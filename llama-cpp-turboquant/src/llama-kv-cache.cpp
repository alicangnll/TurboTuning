...
llama_kv_cache::llama_kv_cache(
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        //   9 = JSON per-layer config from TURBO_LAYER_CONFIG_FILE
        //       Format: {"k":["turbo4","turbo2",...], "v":["q8_0","turbo2",...]}
        //       Each array entry = type for that layer (by index, 0-based)
        ...
        ggml_type layer_type_v = type_v;
        {
            ...
            } else if (adaptive_mode == 9) {
                // JSON per-layer config: read TURBO_LAYER_CONFIG_FILE once, apply per layer
                // Format: {"k":["turbo4","turbo2",...], "v":["q8_0","turbo2",...]}
                static auto parse_layer_config = [&]() -> std::pair<std::vector<ggml_type>, std::vector<ggml_type>> {
                    auto str_to_ggml_type = [](const std::string & s) -> ggml_type {
                        if (s == "turbo4" || s == "turbo4_0") return GGML_TYPE_TURBO4_0;
                        if (s == "turbo3" || s == "turbo3_0") return GGML_TYPE_TURBO3_0;
                        if (s == "turbo2" || s == "turbo2_0") return GGML_TYPE_TURBO2_0;
                        ...
                        return GGML_TYPE_COUNT; // unknown
                    };
                    auto parse_string_array = [](const std::string & json, const std::string & key) -> std::vector<std::string> {
                    ...
                    const char * cfg_file = getenv("TURBO_LAYER_CONFIG_FILE");
                    if (!cfg_file) {
                        LLAMA_LOG_WARN("llama_kv_cache: mode 9 requires TURBO_LAYER_CONFIG_FILE env var\n");
                        return {};
                    }
                    std::ifstream f(cfg_file);
                    if (!f.is_open()) {
                        LLAMA_LOG_WARN("llama_kv_cache: cannot open TURBO_LAYER_CONFIG_FILE=%s\n", cfg_file);
                        return {};
                    }
                    ...
                    auto k_strs = parse_string_array(json, "k");
                    auto v_strs = parse_string_array(json, "v");
                    std::vector<ggml_type> k_types, v_types;
                    for (auto & s : k_strs) { k_types.push_back(str_to_ggml_type(s)); }
                    for (auto & s : v_strs) { v_types.push_back(str_to_ggml_type(s)); }
                    LLAMA_LOG_INFO("llama_kv_cache: JSON per-layer config loaded: %zu K-types, %zu V-types from %s\n",
                                   k_types.size(), v_types.size(), cfg_file);
                    return {k_types, v_types};
                };
                static const auto layer_cfg = parse_layer_config();
                const auto & k_types = layer_cfg.first;
                const auto & v_types = layer_cfg.second;
                if (il < k_types.size() && k_types[il] != GGML_TYPE_COUNT) {
                    layer_type_k = k_types[il];
                }
                if (il < v_types.size() && v_types[il] != GGML_TYPE_COUNT) {
                    layer_type_v = v_types[il];
                }
            }
        }
        ...
        // Read lazy_window from env once (static), then store on the cache struct.
        // TURBO_LAZY_WINDOW=N: last N cells stay in f16 (hot), older cells in turbo2 (cold).
        // Only activates when the base type is a turbo type.
        if (lazy_window == 0) {
            static const uint32_t env_lazy = [&]() -> uint32_t {
                const char * e = getenv("TURBO_LAZY_WINDOW");
                return e ? (uint32_t)atoi(e) : 0u;
            }();
            lazy_window = env_lazy;
            if (lazy_window > 0 && il == 0) {
                LLAMA_LOG_INFO("llama_kv_cache: lazy encoding enabled: hot=%u (f16) + cold=%u (turbo2)\n",
                               lazy_window, kv_size > lazy_window ? kv_size - lazy_window : 0);
            }
        }

        const bool use_lazy = lazy_window > 0 && lazy_window < kv_size
                           && (k_is_turbo || v_is_turbo);

        // Cold zone: turbo2 for positions [0, kv_size - lazy_window)
        // Hot  zone: f16   for positions [kv_size - lazy_window, kv_size)
        const uint32_t cold_size = use_lazy ? (kv_size - lazy_window) : kv_size;
        const ggml_type cold_type_k = use_lazy ? GGML_TYPE_TURBO2_0 : layer_type_k;
        const ggml_type cold_type_v = use_lazy ? GGML_TYPE_TURBO2_0 : layer_type_v;

        ggml_tensor * k = has_k ? ggml_new_tensor_3d(ctx, cold_type_k, n_embd_k_gqa_eff, cold_size, n_stream) : nullptr;
        ggml_tensor * v = has_v ? ggml_new_tensor_3d(ctx, cold_type_v, n_embd_v_gqa_eff, cold_size, n_stream) : nullptr;
        ...
        for (uint32_t s = 0; s < n_stream; ++s) {
            k_stream.push_back(has_k ? ggml_view_2d(ctx, k, n_embd_k_gqa_eff, cold_size, k->nb[1], s*k->nb[2]) : nullptr);
            v_stream.push_back(has_v ? ggml_view_2d(ctx, v, n_embd_v_gqa_eff, cold_size, v->nb[1], s*v->nb[2]) : nullptr);
        }

        // Hot zone allocation (f16, lazy_window rows)
        ggml_tensor * k_hot = nullptr;
        ggml_tensor * v_hot = nullptr;
        std::vector<ggml_tensor *> k_hot_stream;
        std::vector<ggml_tensor *> v_hot_stream;
        if (use_lazy) {
            k_hot = has_k ? ggml_new_tensor_3d(ctx, GGML_TYPE_F16, n_embd_k_gqa_eff, lazy_window, n_stream) : nullptr;
            v_hot = has_v ? ggml_new_tensor_3d(ctx, GGML_TYPE_F16, n_embd_v_gqa_eff, lazy_window, n_stream) : nullptr;
            has_k && ggml_format_name(k_hot, "cache_k_hot_l%d", il);
            has_v && ggml_format_name(v_hot, "cache_v_hot_l%d", il);
            for (uint32_t s = 0; s < n_stream; ++s) {
                k_hot_stream.push_back(has_k ? ggml_view_2d(ctx, k_hot, n_embd_k_gqa_eff, lazy_window, k_hot->nb[1], s*k_hot->nb[2]) : nullptr);
                v_hot_stream.push_back(has_v ? ggml_view_2d(ctx, v_hot, n_embd_v_gqa_eff, lazy_window, v_hot->nb[1], s*v_hot->nb[2]) : nullptr);
            }
        }
        ...
        // TurboQuant: Natural Space Fix — Rotation matrices DISABLED to prevent graph-side mismatch
        /*
    ...
    for (auto & [buft, ctx] : ctx_map) {
        ...
        if (turbo_rotation != nullptr && turbo_rotation->buffer != nullptr && !model.hparams.no_alloc) {
            ...
            // Note: dequantization kernels now handle their own normalization.
...
ggml_tensor * llama_kv_cache::get_k(ggml_context * ctx, int32_t il, uint32_t n_kv, const slot_info & sinfo) const {
    ...
    // Lazy encoding: cold (turbo2) + hot (f16) — cast both to F16 and concat.
    // The resulting F16 tensor covers [cold_cells | hot_cells] = full kv_size.
    if (k_hot != nullptr) {
        const uint64_t cold_size = (uint64_t) k->ne[1];  // kv_size - lazy_window
        const uint64_t hot_size  = (uint64_t) k_hot->ne[1];  // lazy_window
        GGML_UNUSED(hot_size);

        // cold: view into the stream's portion, cast to F16
        ggml_tensor * k_cold_view = ggml_view_3d(ctx, k,
            n_embd_k_gqa, cold_size, ns,
            k->nb[1],
            k->nb[1] * cold_size,
            k->nb[1] * cold_size * sinfo.s0);
        ggml_tensor * k_cold_f16 = ggml_cast(ctx, k_cold_view, GGML_TYPE_F16);

        // hot: already F16
        ggml_tensor * k_hot_view = ggml_view_3d(ctx, k_hot,
            n_embd_k_gqa, k_hot->ne[1], ns,
            k_hot->nb[1],
            k_hot->nb[1] * k_hot->ne[1],
            k_hot->nb[1] * k_hot->ne[1] * sinfo.s0);

        // concat along dim 1 (sequence cells), then reshape to 4D for attention
        ggml_tensor * k_full = ggml_cont(ctx, ggml_concat(ctx, k_cold_f16, k_hot_view, 1));
        const uint64_t full_kv = k_full->ne[1];
        GGML_UNUSED(full_kv);

        return ggml_reshape_4d(ctx, k_full,
