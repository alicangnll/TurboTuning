...
public:
    ...
    // Accessors for internal layer tensors (used by LLMTuning)
    ggml_tensor * get_layer_k(int32_t il) const override { return layers[map_layer_ids.at(il)].k; }
    ggml_tensor * get_layer_v(int32_t il) const override { return layers[map_layer_ids.at(il)].v; }
...
private:
    ...
    struct kv_layer {
        ...
        // Lazy encoding: hot zone (f16, recent window_size cells) + cold zone (turbo2, rest).
        // Null when TURBO_LAZY_WINDOW is not set (normal path, no overhead).
        ggml_tensor * k_hot = nullptr;  // f16, [n_embd_k_gqa, lazy_window]
        ggml_tensor * v_hot = nullptr;  // f16, [n_embd_v_gqa, lazy_window]
    ...
    // Lazy encoding: 0 = disabled. When >0, the last `lazy_window` cells of each
    // layer are stored in f16 (hot zone) and the rest in turbo2 (cold zone).
    // Set via env var TURBO_LAZY_WINDOW=<n> (e.g. 64).
    uint32_t lazy_window = 0;
